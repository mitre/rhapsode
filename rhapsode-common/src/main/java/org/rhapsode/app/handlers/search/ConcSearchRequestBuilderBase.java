...
public class ConcSearchRequestBuilderBase extends BaseRequestBuilder {
    public void extractConcBase(RhapsodeSearcherApp searcherApp, HttpServletRequest servletRequest,
        ...
        request.setTokensBefore(
                ParamUtil.getInt(servletRequest.getParameter(
                        C.WORDS_BEFORE),
                        searcherApp.getSessionManager().getDynamicParameterConfig().getInt(
                                DynamicParameters.CONC_WORDS_BEFORE),
                        0,
                        Integer.MAX_VALUE));
        request.setTokensAfter(
                ParamUtil.getInt(servletRequest.getParameter(
                        C.WORDS_AFTER),
                        searcherApp.getSessionManager().getDynamicParameterConfig().getInt(
                                DynamicParameters.CONC_WORDS_AFTER),
                        0,
                        Integer.MAX_VALUE));
        request.setMaxStoredWindows(
                ParamUtil.getInt(servletRequest.getParameter(
                        C.MAX_STORED_WINDOWS),
                        searcherApp.getSessionManager().getDynamicParameterConfig().getInt(
                                DynamicParameters.CONC_MAX_STORED_WINDOWS),
                        0,
                        Integer.MAX_VALUE));
        request.setIgnoreDuplicateWindows(
                ParamUtil.getBoolean(
                        servletRequest.getParameter(C.IGNORE_DUPLICATE_WINDOWS),
                        searcherApp.getSessionManager().getDynamicParameterConfig().getBoolean(
                                DynamicParameters.CONC_IGNORE_DUPLICATE_WINDOWS)));
